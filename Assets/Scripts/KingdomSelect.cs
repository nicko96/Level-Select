...
public class KingdomSelect : MonoBehaviour {
    ...
    void Update () {
        if (Input.GetKeyDown(KeyCode.RightArrow) && currentKingdom < kingdoms.Count)
        {
            currentKingdom++;
            LookAtKingdom(kingdoms[currentKingdom]);
        }
        else if (Input.GetKeyDown(KeyCode.LeftArrow) && currentKingdom >= 0)
        {
            currentKingdom--;
            LookAtKingdom(kingdoms[currentKingdom]);
        }
